...
# Usage: $(title 'text') e.g. $(title 'Learn FCSS from the Best')
title () {
    echo "<title>$1</title>"
}
# Usage: $(abbr title initials) e.g. $(abbr 'Functional CSS' FCSS)
abbr () {
  echo "<abbr title=\"$1\">$2</abbr>"
}
# Usage: $(b term) e.g. $(b awesome)
b () {
  echo "<b class=\"\">$1</b>"
}

# Usage: $(strong text) e.g. $(strong 'bold it')
strong () {
  echo "$(strong 'bold it')"
}
